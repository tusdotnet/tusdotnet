...
namespace AspNetCore_netcoreapp3._1_TestApp
{
    public class MyTusHandler : TusHandler
    {
        ...
        public override async Task<CreateFileProcedureResponse> CreateFile(CreateFileContext context)
        {
            ...
            _logger.LogInformation("CreateFile: File created? {Success}", response.Status == System.Net.HttpStatusCode.Created);
        ...
        {
            _logger.LogInformation("WriteData: Receiving upload, starting at {UploadOffset}", context.Headers.UploadOffset);

            var response = await base.WriteData(context);

            _logger.LogInformation("WriteData: Was success? {Success}", response.Status == System.Net.HttpStatusCode.Created);
        ...
        {
            _logger.LogInformation("RetrieveOffset: Retrieving offset for {UploadToken}", context.Headers.ResourceId);

            var response = await base.RetrieveOffset(context);

            _logger.LogInformation("RetrieveOffset: Offset is {UploadOffset}", response.UploadOffset);
        ...
        {
            _logger.LogInformation("Delete: Deleting file {UploadToken}", context.Headers.ResourceId);

            var response = await base.Delete(context);

            _logger.LogInformation("Delete: File deleted? {Deleted}", response.Status == System.Net.HttpStatusCode.NoContent);
