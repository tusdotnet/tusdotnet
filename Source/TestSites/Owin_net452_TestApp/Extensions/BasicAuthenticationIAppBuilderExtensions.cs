    ...
    public static class BasicAuthenticationIAppBuilderExtensions
    {
        public static void SetupSimpleBasicAuth(this IAppBuilder app)
        {
            ...
            app.Use(async (context, next) =>
            {
                ...
                var parts = Encoding.UTF8.GetString(Convert.FromBase64String(basicAuthHeader.Substring(BasicSchemeName.Length))).Split(new[] { ':' }, StringSplitOptions.RemoveEmptyEntries);
                if (parts.Length != 2)
                {
                ...
                if (parts[0] == Username && parts[1] == Password)
                {
                    ...
                    context.Authentication.User = new ClaimsPrincipal(new ClaimsIdentity(claims, BasicSchemeName));
                    await next();
                    return;
                }
