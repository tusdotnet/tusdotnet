...
Function RunTests([string]$sdkVersion, [string[]]$runtimes)
{
    ...
    Foreach ($item in $runtimes) {
        dotnet test -c Release -v m --nologo --no-build -f $item
    }
...
dotnet build -c release -v q -nowarn:NETSDK1138 -nowarn:NU1902 -nowarn:NU1903 -nowarn:NETSDK1215 -nowarn:NU1904 -nowarn:ASPDEPR004
