...
namespace tusdotnet
{
	internal class TusMiddleware : OwinMiddleware
	{
		...
		public override Task Invoke(IOwinContext context)
		{
			...
			if (!context.Request.Headers.ContainsKey(HeaderConstants.TusResumable))
			{
				return Next.Invoke(context);
			}
			...
			if (tusResumable != HeaderConstants.TusResumableValue)
			{
				...
				return context.Response.WriteAsync($"Tus version {tusResumable} is not supported.");
			}
			...
			switch (context.Request.Method.ToLower())
			{
				...
				default:
					throw new NotImplementedException($"HTTP method {context.Request.Method.ToLower()} is not yet implemented.");
			}
			// TODO: Error handling.			
		}
		...
		private Task HandlePatchRequest(IOwinContext context)
		{
			 ...
			 * They MAY do so by checking for read/write socket errors, as well as setting read/write timeouts. A timeout SHOULD be handled by closing the underlying connection.
		...
		private Task HandleHeadRequest(IOwinContext context)
		{
			/*
		...
		private bool ShouldHandleRequest(IOwinContext context)
		{
			return context.Request.Uri.LocalPath.StartsWith(_config.UrlPath, StringComparison.InvariantCultureIgnoreCase)
				   && SupportedMethods.Contains(context.Request.Method.ToLower());
		}
	}
}
