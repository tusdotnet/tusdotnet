    ...
    internal class WriteFileHandler : IntentHandler
    {
        ...
        internal override async Task Invoke()
        {
            ...
            var expires = _expirationHelper.IsSlidingExpiration
                ? await _expirationHelper.SetExpirationIfSupported(
                    Context.FileId,
                    Context.CancellationToken
                )
                : await _expirationHelper.GetExpirationIfSupported(
                    Context.FileId,
                    Context.CancellationToken
                );

            var matchChecksumResult = await _checksumHelper.MatchChecksum(
        ...
        private async Task<long> HandlePipelineWrite()
        {
            long bytesWritten;
            var isSupported =
                Context.Configuration.UsePipelinesIfAvailable && StoreAdapter.Features.Pipelines;
            if (isSupported)
            {
                var pipeReader = await GuardedPipeReaderFactory.Create(Context);
                bytesWritten = await StoreAdapter.AppendDataAsync(
                    Context.FileId,
                    pipeReader,
                    CancellationToken
                );
        ...
        private async Task<long> HandleStreamWrite()
        {
            ...
            var bytesWritten = await Store.AppendDataAsync(
                Context.FileId,
                stream,
                Context.CancellationToken
            );
        ...
        private Task WriteUploadLengthIfDefered()
        {
            ...
            {
                return StoreAdapter.SetUploadLengthAsync(
                    Context.FileId,
                    long.Parse(uploadLengthHeader),
                    Context.CancellationToken
                );
            }
