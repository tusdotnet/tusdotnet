...
namespace tusdotnet.test.Extensions
{
    internal static class ITusStoreExtensions
    {
        ...
        internal static ITusStore WithAppendDataDrainingTheRequestBody(this ITusStore store, string fileId)
        {
            return store.WithAppendDataCallback(fileId, async ci =>
            {
        ...
        internal static ITusStore WithAppendDataCallback(this ITusStore store, string fileId, Func<CallInfo, Task<long>> onInvokeAppendData)
        {
            store.AppendDataAsync(fileId, Arg.Any<Stream>(), Arg.Any<CancellationToken>()).Returns(onInvokeAppendData);

#if pipelines

            if (store is ITusPipelineStore pipelineStore)
            {
                pipelineStore.AppendDataAsync(fileId, Arg.Any<System.IO.Pipelines.PipeReader>(), Arg.Any<CancellationToken>()).Returns(onInvokeAppendData);
            }
#endif
        ...
        internal static ITusStore WithSetUploadLengthCallback(this ITusStore store, string fileId, Action<long> callback)
        {
