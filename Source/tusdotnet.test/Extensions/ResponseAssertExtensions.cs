...
namespace tusdotnet.test.Extensions
{
    ...
    {
        internal static async Task ShouldBeErrorResponse(
        ...
        internal static void ShouldContainHeader(
            this HttpResponseMessage response,
            string headerName,
            string headerValue
        )
        {
            response
                ...
                .ShouldBeTrue("Header " + headerName + " was not found in response");
            var value = response.Headers.GetValues(headerName).ToList();
            value.Count.ShouldBe(1);
            value[0].ShouldBe(headerValue);
        }

        internal static void ShouldNotContainHeaders(
            this HttpResponseMessage response,
            params string[] headerNames
        )
        {
            var allHeaders = response
                .Headers.Concat(response.Content?.Headers)
                .ToDictionary(f => f.Key, null);

            foreach (var item in headerNames)
            {
                allHeaders
                    .ContainsKey(item)
                    .ShouldBeFalse(item + " existed in response but should not have");
            }
        }
    }
}
