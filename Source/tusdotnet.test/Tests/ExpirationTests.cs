...
namespace tusdotnet.test.Tests
{
    public class ExpirationTests
    {
        ...
        public async Task Patch_Requests_Contain_Upload_Expires_Header_For_Normal_Uploads_If_Expiration_Is_Configured(
            string methodToUse
        )
        {
            var store = Substitute
                .For<ITusStore, ITusExpirationStore>()
        ...
        {
            var store = Substitute
                .For<ITusStore, ITusExpirationStore, ITusConcatenationStore>()
                .WithExistingFile("expirationtestfile", 10, 3);
            var expirationStore = (ITusExpirationStore)store;
        ...
        [Theory, XHttpMethodOverrideData]
        public async Task Post_Requests_Contain_Upload_Expires_Header_For_Normal_Uploads_If_Expiration_Is_Configured(
            string methodToUse
        )
        {
            ...
            response.ShouldContainHeader(
                "Upload-Expires",
                now.Add(TimeSpan.FromMinutes(10)).ToString("r")
            );
        }
        ...
        )
        {
            ...
            concatenationStore
                .CreateFinalFileAsync(null, null, CancellationToken.None)
                .ReturnsForAnyArgs(Guid.NewGuid().ToString());
        ...
        [Theory, XHttpMethodOverrideData]
        public async Task Upload_Expires_Header_Is_Updated_On_Patch_Requests_If_Sliding_Expiration_Is_Configured(
            string methodToUse
        )
        {
            ...
            expirationStore
                .GetExpirationAsync("expirationtestfile", Arg.Any<CancellationToken>())
                .ReturnsForAnyArgs(_ => expires);
            expirationStore
                .SetExpirationAsync(
                    "expirationtestfile",
        ...
        [Theory, XHttpMethodOverrideData]
        public async Task Upload_Expires_Header_Is_Not_Updated_On_Patch_Requests_If_Absolute_Expiration_Is_Configured(
            string methodToUse
        )
        {
            var store = Substitute.For<ITusStore, ITusExpirationStore>();
            ...
            store.GetUploadOffsetAsync(null, CancellationToken.None).ReturnsForAnyArgs(_ => offset);
            store
                .AppendDataAsync("testexpiration", Arg.Any<Stream>(), Arg.Any<CancellationToken>())
                .Returns(_ =>
                {
                    offset += 3;
                    return 3;
            ...
            expirationStore
                .GetExpirationAsync("testexpiration", Arg.Any<CancellationToken>())
                .ReturnsForAnyArgs(expires);
            await expirationStore.SetExpirationAsync(
                "testexpiration",
                Arg.Any<DateTimeOffset>(),
                Arg.Any<CancellationToken>()
            );
        ...
        [Theory, XHttpMethodOverrideData]
        public async Task Requests_Return_404_NotFound_If_Upload_Expires_Header_Has_Expired(
            string methodToUse
        )
        {
        ...
        [Theory, XHttpMethodOverrideData]
        public async Task Upload_Expires_Header_Is_Not_Set_If_Expiration_Extension_Is_Disabled(
            string methodToUse
        )
        {
            var store = MockStoreHelper.CreateWithExtensions<
        ...
        [Theory, XHttpMethodOverrideData]
        public async Task Upload_Expires_Header_Is_Not_Updated_On_Patch_If_Expiration_Extension_Is_Disabled(
            string methodToUse
        )
        {
            var store = MockStoreHelper
                .CreateWithExtensions<ITusCreationStore, ITusExpirationStore>()
                .WithExistingFile("file", 100, 0);
            var expirationStore = (ITusExpirationStore)store;
