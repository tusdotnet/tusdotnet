    ...
    public class WriteFilePipelinesTests
    {
        ...
        public async Task Ignores_Request_If_Url_Does_Not_Match()
        {
            using var server = TestServerFactory.CreateWithForwarding(
                Substitute.For<ITusPipelineStore>(),
        ...
        )
        {
            var store = (ITusPipelineStore)
                Substitute.For<ITusPipelineStore>().WithExistingFile("testfile");
        ...
        public async Task Returns_204_No_Content_On_Success(string methodToUse)
        {
            var store = (ITusPipelineStore)
                Substitute
                    .For<ITusPipelineStore>()
                    .WithExistingFile("testfile", uploadLength: 10, uploadOffset: 0);
            store
                ...
                var response = await server
                    ...
                    .And(m =>
                    {
                        m.Content = new StreamContent(
                            new SlowMemoryStream(new byte[5], delayPerReadInMs: 500)
                        );
        ...
        public async Task Runs_OnFileCompleteAsync_And_OnUploadCompleteAsync_When_Upload_Is_Complete()
        {
            ...
            store
                .AppendDataAsync("file1", Arg.Any<PipeReader>(), Arg.Any<CancellationToken>())
                .Returns(3)
                .AndDoes(_ => firstOffset += 3);

            store
                .AppendDataAsync("file2", Arg.Any<PipeReader>(), Arg.Any<CancellationToken>())
                .Returns(3)
...
#pragma warning restore CS0618 // Type or member is obsolete
        ...
        public async Task Does_Not_Run_OnFileCompleteAsync_Nor_OnUploadCompleteAsync_If_Upload_Length_Is_Zero()
        {
            ...
            var store = (ITusPipelineStore)
                Substitute.For<ITusPipelineStore>().WithExistingFile("file1", _ => 0, _ => 0);
...
#pragma warning restore CS0618 // Type or member is obsolete
        ...
        public async Task A_TusConfigurationException_Is_Thrown_If_File_Was_Created_Using_UploadDeferLength_But_The_Store_Used_For_Writing_Data_Does_Not_Support_UploadDeferLength()
        {
            ...
            var storeWithoutUploadDeferLength = (ITusPipelineStore)
                Substitute
                    .For<ITusPipelineStore>()
            ...
            using (
                var server = TestServerFactory.Create(
                    creationStoreWithUploadDeferLength,
                    usePipelinesIfAvailable: true
                )
            )
            {
