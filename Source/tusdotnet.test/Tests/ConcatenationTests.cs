    ...
    public class ConcatenationTests
    {
        ...
        )
        {
            ...
            var response = await server
                ...
                .AddHeader(
                    "Upload-Concat",
                    $"final;/files/partial1 {server.BaseAddress.ToString().TrimEnd('/')}/files/partial2"
                )
                .OverrideHttpMethodIfNeeded("POST", methodToUse)
                .SendAsync(methodToUse);
        ...
        [Theory, XHttpMethodOverrideData]
        public async Task Returns_400_Bad_Request_If_Upload_Concat_Header_Is_Unparsable_Or_Not_Final_Nor_Partial(
            string methodToUse
        )
        {
            using var server = TestServerFactory.Create(
                MockStoreHelper.CreateWithExtensions<ITusCreationStore, ITusConcatenationStore>()
            );
        ...
        [Theory, XHttpMethodOverrideData]
        public async Task Head_Request_To_Final_File_Does_Not_Contain_Upload_Offset_Header_Until_Completed(
            string methodToUse
        )
        {
            var uploadOffset = 10;
            ...
            store
                .GetUploadOffsetAsync("finalconcat", Arg.Any<CancellationToken>())
                .Returns(_ => uploadOffset);

            ((ITusConcatenationStore)store)
                .GetUploadConcatAsync("finalconcat", Arg.Any<CancellationToken>())
                .Returns(_ => new FileConcatFinal("1", "2"));
        ...
        [Theory, XHttpMethodOverrideData]
        public async Task Head_Request_To_Partial_File_Contains_Upload_Offset_Header(
            string methodToUse
        )
        {
            var store = MockStoreHelper.CreateWithExtensions<
            ...
            ((ITusConcatenationStore)store)
                .GetUploadConcatAsync("partialconcat", Arg.Any<CancellationToken>())
        ...
        [Theory, XHttpMethodOverrideData]
        public async Task Head_Request_To_Final_File_Contains_Upload_Concat_Header(
            string methodToUse
        )
        {
            ...
            ((ITusConcatenationStore)store)
                .GetUploadConcatAsync(null, CancellationToken.None)
        ...
        [Theory, XHttpMethodOverrideData]
        public async Task Head_Request_To_Partial_File_Contains_Upload_Concat_Header(
            string methodToUse
        )
        {
            var store = MockStoreHelper.CreateWithExtensions<ITusConcatenationStore>();
        ...
        [Theory, XHttpMethodOverrideData]
        public async Task Head_Request_To_Regular_File_Does_Not_Contain_Upload_Concat_Header(
            string methodToUse
        )
        {
            var store = MockStoreHelper.CreateWithExtensions<ITusConcatenationStore>();
            ...
            ((ITusConcatenationStore)store)
                .GetUploadConcatAsync(null, CancellationToken.None)
                .ReturnsForAnyArgs(null as FileConcat);
        ...
        public async Task Returns_403_Forbidden_When_Patching_A_Final_File(string methodToUse)
        {
            ...
            await response.ShouldBeErrorResponse(
                ...
                "File with \"Upload-Concat: final\" cannot be patched"
            );
        }
        ...
        public async Task Partial_File_Metadata_Is_Not_Transfered_To_Final_File(string methodToUse)
        {
            ...
            creationStore
                .GetUploadMetadataAsync("partial1", Arg.Any<CancellationToken>())
                .Returns("metaforpartial1");
            creationStore
                .GetUploadMetadataAsync("partial2", Arg.Any<CancellationToken>())
                .Returns("metaforpartial2");
            ...
            await concatStore
                .Received()
                .CreateFinalFileAsync(Arg.Any<string[]>(), null, Arg.Any<CancellationToken>());
        }

        [Theory, XHttpMethodOverrideData]
        public async Task Returns_400_Bad_Request_If_Any_Of_The_Partial_Files_Are_Not_Found(
            string methodToUse
        )
        {
            var store = MockStoreHelper.CreateWithExtensions<
            ...
            var concatStore = (ITusConcatenationStore)store;
            concatStore
                .CreateFinalFileAsync(null, null, Arg.Any<CancellationToken>())
        ...
        [Theory, XHttpMethodOverrideData]
        public async Task Returns_400_Bad_Request_If_Any_Of_The_Partial_Files_Are_Not_Partial(
            string methodToUse
        )
        {
            var store = MockStoreHelper.CreateWithExtensions<
            ...
            concatStore
                .GetUploadConcatAsync("partial1", Arg.Any<CancellationToken>())
                .Returns(new FileConcatPartial());
            concatStore
                .GetUploadConcatAsync("partial2", Arg.Any<CancellationToken>())
                .Returns(null as FileConcat);
        ...
        [Theory, XHttpMethodOverrideData]
        public async Task Returns_400_Bad_Request_If_Any_Partial_File_Is_Not_Complete(
            string methodToUse
        )
        {
            var store = MockStoreHelper.CreateWithExtensions<
        ...
        {
            var store = CreateStoreForPartialFileConcatenation(
                returnThisPartialFileIdOnCreate: "partial1"
            );
            var concatStore = (ITusConcatenationStore)store;
            ...
            await concatStore
                .Received()
                .CreatePartialFileAsync(1, null, Arg.Any<CancellationToken>());
        }

        [Theory, XHttpMethodOverrideData]
        public async Task Returns_413_Request_Entity_Too_Large_If_Final_File_Size_Exceeds_Tus_Max_Size(
            string methodToUse
        )
        {
            var store = CreateStoreForFinalFileConcatenation();
...
#pragma warning restore CS0618 // Type or member is obsolete
        ...
        [Theory, XHttpMethodOverrideData]
        public async Task Does_Not_Run_OnFileCompleteAsync_When_A_Partial_File_Is_Created(
            string methodToUse
        )
        {
            var store = MockStoreHelper.CreateWithExtensions<
            ...
            store
                .AppendDataAsync("partial1", Arg.Any<Stream>(), Arg.Any<CancellationToken>())
                .Returns(1);

            var concatStore = (ITusConcatenationStore)store;
            concatStore
                .CreatePartialFileAsync(1, null, CancellationToken.None)
                .ReturnsForAnyArgs("partial1");
            concatStore
                .GetUploadConcatAsync("partial1", Arg.Any<CancellationToken>())
                .Returns(new FileConcatPartial());
...
#pragma warning restore CS0618 // Type or member is obsolete
        ...
        [Theory, XHttpMethodOverrideData]
        public async Task OnBeforeCreateAsync_Receives_FileConcatPartial_For_Partial_Files(
            string methodToUse
        )
        {
            ...
            using var server = TestServerFactory.Create(
                CreateStoreForPartialFileConcatenation(),
                new Events
                {
        ...
        [Theory, XHttpMethodOverrideData]
        public async Task OnBeforeCreateAsync_Receives_FileConcatFinal_For_Final_Files(
            string methodToUse
        )
        {
            FileConcat fileConcat = null;
        ...
        [Theory, XHttpMethodOverrideData]
        public async Task OnCreateCompleteAsync_Receives_FileConcatPartial_For_Partial_Files(
            string methodToUse
        )
        {
            FileConcat fileConcat = null;
        ...
        [Theory, XHttpMethodOverrideData]
        public async Task OnCreateCompleteAsync_Receives_FileConcatFinal_For_Final_Files(
            string methodToUse
        )
        {
            FileConcat fileConcat = null;
        ...
        public async Task OnAuthorized_Is_Called_For_Partial_Files()
        {
            ...
            using var server = TestServerFactory.Create(
                CreateStoreForPartialFileConcatenation("partial1"),
                new Events
                {
        ...
        private static ITusStore CreateStoreForPartialFileConcatenation(
            string returnThisPartialFileIdOnCreate = null
        )
        {
            var store = MockStoreHelper.CreateWithExtensions<
                ITusCreationStore,
            ...
            concatStore
                .CreatePartialFileAsync(-1, null, CancellationToken.None)
                .ReturnsForAnyArgs(returnThisPartialFileIdOnCreate ?? Guid.NewGuid().ToString());
