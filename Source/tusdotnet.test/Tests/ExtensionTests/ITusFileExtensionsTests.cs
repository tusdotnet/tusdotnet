...
namespace tusdotnet.test.Tests.ExtensionTests
{
    public class ITusFileExtensionsTests
    {
        ...
        public async Task Returns_Correct_File_Upload_Status(int offset, int length, bool expectedCompleted)
        {
            var file = Substitute.For<ITusFile>();
            var store = Substitute.For<ITusStore>();
            store.GetUploadLengthAsync(Arg.Any<string>(), Arg.Any<CancellationToken>()).Returns(length);
            store.GetUploadOffsetAsync(Arg.Any<string>(), Arg.Any<CancellationToken>()).Returns(offset);
