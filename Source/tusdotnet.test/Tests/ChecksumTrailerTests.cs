...
namespace tusdotnet.test.Tests
{
    public class ChecksumTrailerTests
    {
        ...
        public async Task Returns_204_No_Content_If_ChecksumTrailer_Extension_Is_Disabled()
        {
            ...
            using var server = CreateTestServerWithChecksumTrailer(
                store,
            ...
            await store
                ...
                .VerifyChecksumAsync(default, default, default, default);
        }
        ...
        public async Task Returns_400_Bad_Request_If_Checksum_Algorithm_Is_Not_Supported_Using_Trailing_Header()
        {
            ...
            await ((ITusStore)store)
                .Received()
                .AppendDataAsync("checksum", Arg.Any<Stream>(), Arg.Any<CancellationToken>());
            ...
            await store
                ...
                .VerifyChecksumAsync(
                    "checksum",
                    "sha1",
                    Arg.Is(GetFallbackChecksumPredicate()),
                    Arg.Any<CancellationToken>()
                );
        }
        ...
        public async Task Returns_400_Bad_Request_If_Checksum_Is_Unparsable_Using_Trailing_Header(
            string uploadChecksumTrailingHeader
        )
        {
        ...
        {
            var checksumStore = CreateStore<ITusChecksumStore>(
                verifyChecksumAsyncReturnValue: false
            );
            using var server = CreateTestServerWithChecksumTrailer(
                checksumStore,
                "sha1 Kq5sNclPz7QV2+lfQIuc6R7o000="
            );
        ...
        public async Task Checksum_Is_Verified_If_Client_Disconnects_Using_Trailing_Header()
        {
            ...
            byte[] checksumArgument = (byte[])
                ((ITusChecksumStore)store)
                    .ReceivedCalls()
                    .First(x =>
                        x.GetMethodInfo().Name == nameof(ITusChecksumStore.VerifyChecksumAsync)
                    )
                    .GetArguments()[2];
            ...
            context.Response.Message.ShouldBe(
                "Header Upload-Checksum does not match the checksum of the file"
            );
        }

        private static TestServer CreateTestServerWithChecksumTrailer(
            ITusChecksumStore store,
            string trailingUploadChecksumValue,
        ...
        private static T CreateStore<T>(
            string supportedAlgorithm = "sha1",
            bool verifyChecksumAsyncReturnValue = true
        )
        {
            ...
            checksumStore
                .GetSupportedAlgorithmsAsync(default)
                .ReturnsForAnyArgs(new[] { supportedAlgorithm });
            checksumStore
                .VerifyChecksumAsync(default, default, default, default)
                .ReturnsForAnyArgs(verifyChecksumAsyncReturnValue);
